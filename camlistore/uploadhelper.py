...
class UploadHelper(object):
    ...
    The upload helper endpoint allows for uploading of whole files into Perkeep,
    with automatic chunking and generation of a file schema blob. This allows
    uploading of files without the need to do chunking/span trees locally.
    """
    ...
    def upload_file(self, filename, fileobj, mtime=None):
        ...
        ``filename`` is the desired file name to be stored in the blob store. It
        cannot be blank. ``fileobj`` is a file-like object containing the 
